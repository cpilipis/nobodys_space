...
{ //the type for the laser the ship fires
  coor velocity;
  coor location;
...
bul fire(ship * p)
{ //Put a shot at the tip of the ship with the ship's velocity direction.
  bul shot;
  //Spawn a bullet at the tip of the ship ship's nose
  ...
  shot.age = 60;
  shot.parent = p; //The bullet now knows the ship's memory addess, so we can increase the ship's score when this bullet hits something
  return shot;
}
...
{ //Check if the shot is inside an asteroid
  ...
  if(isInRadius(shot->location, a->location, a->size) && shot->age != 0)
  {
    //return the split up new asteroids
    shot->age = 0;//kill the shot
    shot->parent->score += 100 - a->size; //reward the ship who fired the asteroid with points
